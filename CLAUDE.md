...
- The font wedge above still slipped through intermittently and hung whole test runs. Defenses, all in place: the CRTRendering tests funnel every renderer/atlas through `RenderTestSupport` (`Tests/CRTRenderingTests/RenderTestSupport.swift`), which once-per-process disables fontd auto-activation, registers the bundled faces, and pre-warms each face before any test runs — then shares one renderer per face so CoreText font creation happens minimally. The rendering suite must stay `--no-parallel`. `Scripts/test.sh` watchdogs the test *run* (not the build): if it overruns it samples the wedged helper to `/tmp/crterminal-test-hang.txt`, kills only our process group, and retries. CI runs the same script (so it inherits `--no-parallel` + watchdog) and caps each job with `timeout-minutes`. If you see a fresh hang, the captured sample in that log is the thing to read.
